...
select count( case when to_date = '9999-01-01' then 1 END) as '현재 근무하고 있는 직원의 수'
from salaries;
...
select count(distinct emp_no) as '급여가 120,000 이상인 사원 수'
from salaries
where salary >= 120000
...
select count(distinct emp_no) as 'Engineer 직책의 사원 수'
from titles
